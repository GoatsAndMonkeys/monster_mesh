...
void add_style_drop_down_style(lv_obj_t *obj) {
    // LVGL dropdown list is positioned relative to its parent; when the parent
    // is inside a scrollable panel the list renders at (0,0). Move it to the
    // top layer so it positions correctly relative to the screen.
    lv_obj_set_parent(lv_dropdown_get_list(obj), lv_layer_top());
};
