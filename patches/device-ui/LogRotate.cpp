...
// MonsterMesh hook: when the emulator is active we skip packet-history
// flash writes. ESP32 cache is disabled across both cores during flash
// program/erase, which pauses the emu + render tasks and shows up as a
// mid-play freeze. Messages still reach the phone over BLE; we just
// don't persist them to LittleFS during gameplay.
extern "C" volatile bool g_mmSuppressFlashWrites;

bool LogRotate::write(const ILogEntry &entry)
{
    if (g_mmSuppressFlashWrites) return true;  // pretend success, drop write
