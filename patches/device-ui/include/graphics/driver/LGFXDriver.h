...
// MonsterMesh hook — set the global LGFX pointer once it's been constructed.
// Declared weak so non-MonsterMesh builds link without it.
extern "C" __attribute__((weak)) void monstermesh_set_lgfx(void *ptr);

template <class LGFX> void LGFXDriver<LGFX>::init(DeviceGUI *gui)
{
    ILOG_DEBUG("LGFXDriver<LGFX>::init...");
    init_lgfx();
    if (monstermesh_set_lgfx) monstermesh_set_lgfx((void *)lgfx);
