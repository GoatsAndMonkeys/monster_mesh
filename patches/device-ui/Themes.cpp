...
static enum Themes::Theme theme = Themes::ePocketGreen;
...
void Themes::set(enum Theme th)
{
    ...
    // Re-theme terminal panel if it exists
    if (objects.mm_terminal_panel) {
        lv_obj_set_style_bg_color(objects.mm_terminal_panel, lv_color_hex(THEME(eMainScreenStyle)), LV_PART_MAIN | LV_STATE_DEFAULT);
    }
    if (objects.mm_terminal_output) {
        lv_obj_set_style_bg_color(objects.mm_terminal_output, lv_color_hex(THEME(eMainScreenStyle)), LV_PART_MAIN | LV_STATE_DEFAULT);
        // Re-color all existing printed labels in terminal output
        uint32_t count = lv_obj_get_child_count(objects.mm_terminal_output);
        for (uint32_t i = 0; i < count; i++) {
            lv_obj_t *child = lv_obj_get_child(objects.mm_terminal_output, i);
            if (child) {
                lv_color_t cur = lv_obj_get_style_text_color(child, LV_PART_MAIN);
                // Separator lines use mid color, regular text uses lightest
                // Detect separators by checking if current color is darker than text
                uint8_t brightness = lv_color_brightness(cur);
                if (brightness < 140)
                    lv_obj_set_style_text_color(child, lv_color_hex(THEME(eNodePanelBorder)), LV_PART_MAIN);
                else
                    lv_obj_set_style_text_color(child, lv_color_hex(THEME(ePanelText)), LV_PART_MAIN);
            }
        }
    }
    if (objects.mm_terminal_input) {
        lv_obj_set_style_bg_color(objects.mm_terminal_input, lv_color_hex(THEME(ePanelPressedBg)), LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_text_color(objects.mm_terminal_input, lv_color_hex(THEME(eBatteryPercentageText)), LV_PART_MAIN | LV_STATE_DEFAULT);
        lv_obj_set_style_border_color(objects.mm_terminal_input, lv_color_hex(THEME(eChannelButtonBorder)), LV_PART_MAIN | LV_STATE_DEFAULT);
    }
}
