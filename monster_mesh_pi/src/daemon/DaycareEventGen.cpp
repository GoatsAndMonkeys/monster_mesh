...
static constexpr uint8_t OUTCOMES_PERSONALITY_COUNT = 4;
    ...
    const DaycarePokemonState *localParty, uint8_t localPartyCount)
{
    ...
    const char *msg;
    if (sp) {
        if (sp->type1 == 2 || sp->type2 == 2) {
            msg = escapeFlying[rngRange(2)];
        } else if (sp->type1 == 4 || sp->type2 == 4) {
            msg = escapeDigger[rngRange(2)];
        } else if (sp->type1 == 7 || sp->type2 == 7) {
            msg = escapeGhost[rngRange(2)];
        } else if (sp->type1 == 9 || sp->type2 == 9) {
    ...
    uint32_t newNodeId)
{
    ...
    // specific); when alone, a species-specific solo vignette. generateComposit-
    // ional heavily favors EVCAT_SOCIAL when neighbors are present (socialW).
    if (neighborCount > 0) {
        if (rngChance(70)) {
...
// ── Super-effective type matchups ─────────────────────────────────────────────

static bool isSuperEffective(uint8_t atkType, uint8_t defType) {
    switch (atkType) {
        case 8:  return defType == 10 || defType == 13 || defType == 6;
        case 9:  return defType == 8  || defType == 4  || defType == 5;
        case 10: return defType == 9  || defType == 4  || defType == 5;
        case 11: return defType == 9  || defType == 2;
        case 13: return defType == 10 || defType == 4  || defType == 2  || defType == 14;
        case 1:  return defType == 0  || defType == 5  || defType == 13;
        case 3:  return defType == 10;
        case 4:  return defType == 8  || defType == 11 || defType == 5  || defType == 3;
        case 2:  return defType == 1  || defType == 10 || defType == 6;
        case 12: return defType == 1  || defType == 3;
        case 5:  return defType == 8  || defType == 13 || defType == 2  || defType == 6;
        case 6:  return defType == 10 || defType == 12;
        case 7:  return defType == 7  || defType == 12;
        case 14: return defType == 14;
