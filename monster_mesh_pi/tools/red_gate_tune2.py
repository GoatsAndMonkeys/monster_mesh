#!/usr/bin/env python3
"""Refined red-gate options. Diagnosis: Gyarados mouth has a dark-red pixel at
hue 13 the current hue<=10 gate misses; Blastoise mouth is ORANGE (hue 19-37),
no true red, so catching it needs either a surgical 'dark-warm' rule or full
orange (which also colors Charizard/Scizor bodies).

Columns: Regular | Current | Hue-widen 16 | +Dark-warm | Allow-orange
"""
...
SIZE = 64; SCALE = 2
VARIANTS = ["Regular", "Current", "Hue-widen 16", "+Dark-warm", "Allow-orange"]
NV = len(VARIANTS)
...
def g_widen(r, g, b):
    hd, s, v = hsv(r, g, b); return s >= 0.45 and (hd <= 16 or hd >= 336)


def g_darkwarm(r, g, b):
    hd, s, v = hsv(r, g, b)
    if s >= 0.45 and (hd <= 16 or hd >= 336): return True         # widened red
    return hd <= 30 and s >= 0.40 and v <= 0.55                   # dark warm accents only


def g_orange(r, g, b):
    hd, s, v = hsv(r, g, b); return s >= 0.40 and (hd <= 34 or hd >= 330)


GATES = [g_current, g_widen, g_darkwarm, g_orange]
...
def main():
    ...
    out = os.path.join(HERE, "..", "..", "sprite-demos", "red_gate_tune2.png"); sheet.save(out)
