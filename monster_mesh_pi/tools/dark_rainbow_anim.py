#!/usr/bin/env python3
"""Dark-Rainbow: DARK greyscale body (gamma 2.1), but the surviving RED pixels
glow with the animated diagonal rainbow instead of staying red. Looping GIF.
"""
...
SAMPLES = [(6, "Charizard"), (126, "Magmar"), (383, "Groudon"), (129, "Magikarp")]
...
def frame(base, off):
    ...
    for y in range(SIZE):
        for x in range(SIZE):
            ...
            if is_red(r, g, b):
                yv = luma(r, g, b)
                hue = 0.85*((x + y)/(2*(SIZE-1))) + off        # diagonal rainbow, scroll
                op[x, y] = (*hue_luma(hue, sat_lite(yv, 0.95), yv), 255)
            else:
                gy = _GAMMA[clamp(luma(r, g, b))]
                op[x, y] = (gy, gy, gy, 255)
    return out
...
def main():
    ...
    for dex, name in SAMPLES:
        ...
        bases.append(reconstruct(idx, palN)); print(f"  loaded {name} ({label})", file=sys.stderr)
