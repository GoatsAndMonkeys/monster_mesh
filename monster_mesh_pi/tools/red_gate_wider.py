#!/usr/bin/env python3
"""Wider-hue sweep. red if sat>=0.45 AND (hue <= LOW or hue >= 336).
Sweep LOW = 16, 20, 26, 30 so we can pick how much mouth-red to grab before
Charizard/Scizor orange bodies start bleeding in.

Blastoise mouth: hue 19-27  |  Charizard body: 20-26  |  Scizor body: 16-27
"""
...
SIZE = 64; SCALE = 2
LOWS = [16, 20, 26, 30]
VARIANTS = ["Regular"] + [f"Hue {lo}" for lo in LOWS]
NV = len(VARIANTS)
...
def is_red(r, g, b, low):
    h, s, v = colorsys.rgb_to_hsv(r/255, g/255, b/255); hd = h*360
    return s >= 0.45 and (hd <= low or hd >= 336)
...
def apply(base, low):
    ...
    for y in range(SIZE):
        for x in range(SIZE):
            r, g, b, a = ip[x, y]
            if a < 128: continue
            if is_red(r, g, b, low): op[x, y] = (r, g, b, 255)
            else:
...
def variants_for(dex):
    ...
    return [base] + [apply(base, lo) for lo in LOWS]
...
def main():
    ...
    out = os.path.join(HERE, "..", "..", "sprite-demos", "red_gate_wider.png"); sheet.save(out)
