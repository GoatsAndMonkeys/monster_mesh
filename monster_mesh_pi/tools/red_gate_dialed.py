#!/usr/bin/env python3
"""Dialed-in combined gate: wider hue + lower sat.
red if sat >= SMIN AND (hue <= LOW or hue >= HIGH).

Targets from pixel diagnosis:
  Blastoise mouth  hue 19-27 (sat 0.47+)
  Gyarados mouth   hue 13 + skin-red hue 20 (sat 0.54)
  Gengar eye spot  hue 25 (sat 0.52)  <- currently missed
Columns: Regular | Current | Dialed A | Dialed B (wider)
"""
...
GATES = [
    ("Current", 0.45, 10, 342),
    ("Dialed A", 0.42, 28, 335),
    ("Dialed B", 0.38, 31, 330),
]
...
def apply(base, smin, low, high):
    ...
    for y in range(SIZE):
        for x in range(SIZE):
            r, g, b, a = ip[x, y]
            if a < 128: continue
            if is_red(r, g, b, smin, low, high): op[x, y] = (r, g, b, 255)
            else:
...
def main():
    ...
    out = os.path.join(HERE, "..", "..", "sprite-demos", "red_gate_dialed.png"); sheet.save(out)
