...
{
    DaycareEvent ev = {};
    rngSeed(millis() ^ (uint32_t)(state.totalEvents * 2654435761UL));
...
// ── Step milestone templates ────────────────────────────────────────────────

static const char *stepWalkTemplates[] = {
    "%s took a leisurely stroll with you.",
    "%s padded along happily beside you.",
    "%s enjoyed the walk, looking around curiously.",
    "%s walked alongside you, tail wagging.",
    "%s stretched its legs on a nice walk.",
};
static constexpr uint8_t STEP_WALK_COUNT = sizeof(stepWalkTemplates) / sizeof(stepWalkTemplates[0]);

static const char *stepRunTemplates[] = {
    "%s sprinted alongside you!",
    "%s raced ahead, then circled back!",
    "%s kept pace with your run, energized!",
    "%s dashed alongside you, loving the speed!",
    "%s bounded ahead with excitement!",
};
static constexpr uint8_t STEP_RUN_COUNT = sizeof(stepRunTemplates) / sizeof(stepRunTemplates[0]);

static const char *stepFriendTemplates[] = {
    "%s went exploring with best friend %s!",
    "%s and %s walked side by side, happy together.",
    "%s and %s trotted along together, inseparable.",
    "%s spotted %s and they walked together.",
};
static constexpr uint8_t STEP_FRIEND_COUNT = sizeof(stepFriendTemplates) / sizeof(stepFriendTemplates[0]);

static const char *stepRivalTemplates[] = {
    "%s and rival %s raced each other!",
    "%s spotted rival %s and picked up the pace!",
    "%s and %s competed to see who could go faster!",
};
static constexpr uint8_t STEP_RIVAL_COUNT = sizeof(stepRivalTemplates) / sizeof(stepRivalTemplates[0]);

static const char *step10kTemplates[] = {
    "All your Pokemon celebrated a big day of adventure!",
    "Your whole team cheered — what a day of walking!",
    "Everyone's tired but happy after a great day!",
};
static constexpr uint8_t STEP_10K_COUNT = sizeof(step10kTemplates) / sizeof(step10kTemplates[0]);

void DaycareEventGen::generateStepEvent(
    DaycareEvent &out,
    const DaycarePokemonState *localParty, uint8_t localPartyCount,
    const DaycareNeighborPokemon *neighbors, uint8_t neighborCount,
    DaycareState &state,
    uint8_t milestone, uint8_t pace, uint16_t streakDays)
{
    memset(&out, 0, sizeof(out));
    out.rarity = RARITY_COMMON;
    if (localPartyCount == 0) return;

    uint8_t targetIdx = pickPokemon(localPartyCount);
    out.targetSpeciesIdx = targetIdx;
    const auto &pkmn = localParty[targetIdx];
    const char *name = pkmn.nickname[0] ? pkmn.nickname : "Pokemon";
    out.xp = 10 * milestone;

    // 10k milestone: celebration, all Pokemon get XP
    if (milestone >= 10) {
        uint32_t seed = millis();
        const char *tmpl = step10kTemplates[seed % STEP_10K_COUNT];
        int pos = snprintf(out.message, sizeof(out.message), "%dk steps: %s", milestone, tmpl);
        if (streakDays > 1) {
            snprintf(out.message + pos, sizeof(out.message) - pos,
                     " That's %d days in a row!", streakDays);
        }
        out.xp = 100;
        out.rarity = RARITY_UNCOMMON;
        return;
    ...
    // 40% chance to involve a neighbor with a relationship
    uint32_t seed = millis();
    if (neighborCount > 0 && (seed % 100) < 40) {
        DaycareRelationship *bestRel = nullptr;
        uint8_t bestNeighborIdx = 0;
        uint8_t bestScore = 0;
        for (uint8_t n = 0; n < neighborCount; n++) {
            for (uint8_t i = 0; i < state.relationshipCount; i++) {
                auto &r = state.relationships[i];
                if (r.nodeId == neighbors[n].nodeId) {
                    uint8_t score = (r.friendship > r.rivalry) ? r.friendship : r.rivalry;
                    if (score > bestScore) { bestScore = score; bestRel = &r; bestNeighborIdx = n; }
                }
            }
        }
        if (bestRel && bestScore >= FRIEND_TIER_ACQUAINTANCE) {
            char neighborName[48];
            const auto &nb = neighbors[bestNeighborIdx];
            if (nb.nickname[0] && strcmp(nb.nickname, "???") != 0)
                snprintf(neighborName, sizeof(neighborName), "%s (%s's Pokemon)", nb.nickname, nb.shortName);
            else
                snprintf(neighborName, sizeof(neighborName), "%s's Pokemon", nb.shortName);

            bool isFriend = bestRel->friendship >= bestRel->rivalry;
            const char *tmpl;
            if (isFriend) {
                tmpl = stepFriendTemplates[seed % STEP_FRIEND_COUNT];
                if (bestRel->friendship <= 253) bestRel->friendship += 2;
                else bestRel->friendship = 255;
            } else {
                tmpl = stepRivalTemplates[seed % STEP_RIVAL_COUNT];
                if (bestRel->rivalry <= 254) bestRel->rivalry += 1;
                else bestRel->rivalry = 255;
            }
            snprintf(out.message, sizeof(out.message), "%dk steps: ", milestone);
            size_t prefixLen = strlen(out.message);
            snprintf(out.message + prefixLen, sizeof(out.message) - prefixLen, tmpl, name, neighborName);
            out.xp += 5;
            return;
        }
    }

    // Solo walking/running event
    const char *tmpl = (pace > 0) ? stepRunTemplates[seed % STEP_RUN_COUNT]
                                  : stepWalkTemplates[seed % STEP_WALK_COUNT];
    snprintf(out.message, sizeof(out.message), "%dk steps: ", milestone);
    size_t prefixLen = strlen(out.message);
    snprintf(out.message + prefixLen, sizeof(out.message) - prefixLen, tmpl, name);
}
