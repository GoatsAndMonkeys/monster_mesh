...
enum DaycareAchievement : uint8_t {
    ...
    // Steps (Pokewalker mode)
    ACH_WEEK_WALKER,        // 7 consecutive days hitting 10k
    ACH_MONTHLY_MILER,      // 30 consecutive days hitting 10k
    ACH_QUARTER_QUEST,      // 90 consecutive days hitting 10k
    ACH_HALF_YEAR_HIKER,    // 180 consecutive days hitting 10k
    ACH_YEAR_LONG_JOURNEY,  // 365 consecutive days hitting 10k
...
static const AchievementDef achievementDefs[] = {
    ...
    // Steps
    {"Week Walker",         "7 consecutive days hitting 10k steps",  false},
    {"Monthly Miler",       "30 consecutive days hitting 10k steps", true},
    {"Quarter Quest",       "90 consecutive days hitting 10k steps", true},
    {"Half-Year Hiker",     "180 consecutive days hitting 10k steps",true},
    {"Year-Long Journey",   "365 consecutive days hitting 10k steps",true},
    // Mesh
...
struct DaycareState {
    uint32_t magic;               // 0xDACA0002 for version check (v2: added steps)
    static constexpr uint32_t MAGIC = 0xDACA0002;
    ...
    uint32_t stepsToday;          // resets at midnight
    uint8_t  lastMilestone;       // last Xk milestone triggered (0-10)
    uint16_t streakDays;          // consecutive days hitting 10k
    uint16_t dayOfYear;           // to detect day rollover (1-366)
