...
// ── Step tracking (Pokewalker mode) ─────────────────────────────────────────

const DaycareEvent &PokemonDaycare::getStepEvent() const { return stepEvent_; }
...
static float gpsDistanceMeters(int32_t lat1E7, int32_t lon1E7, int32_t lat2E7, int32_t lon2E7) {
    ...
    constexpr float EARTH_R = 6371000.0f;
    float lat1 = lat1E7 * 1e-7f * D2R;
    float lat2 = lat2E7 * 1e-7f * D2R;
    float dlat = (lat2E7 - lat1E7) * 1e-7f * D2R;
    float dlon = (lon2E7 - lon1E7) * 1e-7f * D2R;
...
void PokemonDaycare::handleDayRollover(uint16_t currentDayOfYear) {
    if (state_.dayOfYear == 0) { state_.dayOfYear = currentDayOfYear; return; }
    if (currentDayOfYear == state_.dayOfYear) return;
    if (state_.stepsToday >= 10000) state_.streakDays++;
    else state_.streakDays = 0;
    state_.stepsToday = 0;
    state_.lastMilestone = 0;
    state_.dayOfYear = currentDayOfYear;
    if (state_.streakDays >= 7)   grantAchievement(state_, ACH_WEEK_WALKER);
    if (state_.streakDays >= 30)  grantAchievement(state_, ACH_MONTHLY_MILER);
    if (state_.streakDays >= 90)  grantAchievement(state_, ACH_QUARTER_QUEST);
    if (state_.streakDays >= 180) grantAchievement(state_, ACH_HALF_YEAR_HIKER);
    if (state_.streakDays >= 365) grantAchievement(state_, ACH_YEAR_LONG_JOURNEY);
}

uint8_t PokemonDaycare::processStepMilestones() {
    if (!active_ || state_.partyCount == 0) return 0;
    uint8_t currentMilestone = state_.stepsToday / 1000;
    if (currentMilestone > 10) currentMilestone = 10;
    if (currentMilestone <= state_.lastMilestone) return 0;
    uint8_t newMilestones = currentMilestone - state_.lastMilestone;
    uint8_t milestone = currentMilestone;
    state_.lastMilestone = currentMilestone;
    uint8_t pace = (milestone >= 5) ? 1 : 0;
    DaycareEventGen::generateStepEvent(
        stepEvent_, state_.pokemon, state_.partyCount,
        neighbors_, neighborCount_, state_,
        milestone, pace, state_.streakDays);
    if (stepEvent_.xp > 0) {
        if (milestone >= 10) {
            for (uint8_t i = 0; i < state_.partyCount; i++) {
                auto &p = state_.pokemon[i];
                p.totalXpGained += stepEvent_.xp;
                uint16_t levelsNow = p.totalXpGained / XP_PER_LEVEL;
                if (levelsNow > p.totalLevelsGained) p.totalLevelsGained = levelsNow;
            }
        } else {
            auto &p = state_.pokemon[stepEvent_.targetSpeciesIdx];
            p.totalXpGained += stepEvent_.xp;
            uint16_t levelsNow = p.totalXpGained / XP_PER_LEVEL;
            if (levelsNow > p.totalLevelsGained) p.totalLevelsGained = levelsNow;
        }
...
uint8_t PokemonDaycare::updateSteps(int32_t latE7, int32_t lonE7, uint16_t currentDayOfYear) {
    if (!active_) return 0;
    handleDayRollover(currentDayOfYear);
    if (!hasLastGps_) { lastGpsLatE7_ = latE7; lastGpsLonE7_ = lonE7; hasLastGps_ = true; return 0; }
    float dist = gpsDistanceMeters(lastGpsLatE7_, lastGpsLonE7_, latE7, lonE7);
    if (dist < 2.0f) return 0;
    if (dist > 500.0f) { lastGpsLatE7_ = latE7; lastGpsLonE7_ = lonE7; return 0; }
    uint32_t newSteps = (uint32_t)(dist / 0.7f);
    if (newSteps == 0) return 0;
    lastGpsLatE7_ = latE7; lastGpsLonE7_ = lonE7;
    state_.stepsToday += newSteps;
    return processStepMilestones();
}

uint8_t PokemonDaycare::addSteps(uint32_t steps, uint16_t currentDayOfYear) {
    if (!active_ || steps == 0) return 0;
    handleDayRollover(currentDayOfYear);
    state_.stepsToday += steps;
    return processStepMilestones();
}
