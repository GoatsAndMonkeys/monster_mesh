...
#include <Arduino.h>
#include <string.h>
#include "Gen1Learnsets.h"        // gen1MovesLearnedBetween()
...
public:
    ...
    // sram: pointer to the full 32KB SRAM buffer
    ...
    static uint8_t patchPokemon(uint8_t *sram, uint8_t partyIdx, uint8_t dexNum,
                                 uint32_t xpGained) {
        if (partyIdx >= 6 || dexNum == 0 || dexNum > 151) return 0;
    ...
    struct MoveLearnResult {
        ...
        uint8_t pending[8];       // moves that didn't fit (all 4 slots full)
    ...
    // reported in res.pending for the caller to resolve.  Already-known moves
    ...
    // Index (0-3) of the weakest current move, for auto-replacement when the
    // party is full and no interactive chooser is available (the T-Deck patches
    // at ROM-launch with no UI).  Ranks by base power; a 0-power status move is
    // given a nominal score so a useful move (Thunder Wave, Sleep Powder) isn't
    // dropped over a weak attack.  An empty slot always wins.
    static uint8_t weakestMoveSlot(const uint8_t *sram, uint8_t partyIdx) {
        const uint8_t *pkm = &sram[SAV_POKEMON_DATA + partyIdx * SAV_POKEMON_SIZE];
        uint8_t best = 0;
        int bestScore = 1000;
        for (int s = 0; s < 4; s++) {
            uint8_t mv = pkm[PKM_MOVES + s];
            const Gen1MoveData *md = mv ? gen1Move(mv) : nullptr;
            int score = md ? md->power : 0;
            if (score == 0) score = 35;
            if (mv == 0)   score = -1;
            if (score < bestScore) { bestScore = score; best = (uint8_t)s; }
        }
        return best;
    }
